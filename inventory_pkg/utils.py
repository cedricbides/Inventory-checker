    ...
    try:
        with zf.open("xl/sharedStrings.xml") as ss:
            ...
            for si in root.findall("x:si", _NS):
                ...
                direct = si.findtext("x:t", namespaces=_NS)
                if direct is not None:
                    result.append(direct)
                else:
                    parts = [
                        r.findtext("x:t", default="", namespaces=_NS)
                        for r in si.findall("x:r", _NS)
                    ]
...
def _build_cells(row, strings, ns):
    """
    Build a {col_letter: cell_value} dict from a worksheet XML <row> element.

    This is the single canonical implementation of the cell-extraction loop
    that was previously copy-pasted across lazada.py, shopee.py, zalora.py,
    ordazzle.py, and multiple functions in this file.
    """
    cells = {}
...
def _sorted_vals(cells):
    """Return non-empty cell values sorted by column letter."""
    return [v for _, v in sorted(cells.items()) if v]


def append_unique_values(target_list, seen_set, values):
    """
    Append non-empty values into target_list once, preserving input order.

    This centralizes the repeated "if value and value not in seen" pattern
    used across header scanners.
    """
    for value in values:
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as inner:
            ...
            for sheet_path in sheet_names:
                ...
                for i, row in enumerate(root.findall(".//x:row", _NS)):
                    vals = _sorted_vals(_build_cells(row, strings, _NS))
                    if vals:
                        return vals
                    if i >= max_rows:
                        break
...
def detect_channel_from_headers(headers: list) -> "str | None":
    """
    Detect the file type from a list of header strings.

    Returns one of:
      'Channel_Shopee'  — Shopee MASS upload / variant export
      'Channel_Lazada'  — Lazada price/product export
      'Channel_Zalora'  — Zalora SellerStockTemplate
      'ordazzle'        — Ordazzle Unified Inventory export
      'sap'             — SAP article export
      None              — could not determine
    """
    h_lower = {str(col).strip().lower() for col in headers if col}

    if any(k in h_lower for k in ("inv to published stock",
                                   "published stock",
                                   "inventory published",
                                   "node code",
                                   "node name")):
    ...
    if "variant inventory qty" in h_lower or "variant inventory item id" in h_lower:
        return "ordazzle"

    if "article" in h_lower and "plant" in h_lower:
        return "sap"

    if {"product id", "sku.skuid", "sellersku"}.issubset(h_lower):
        return "Channel_Lazada"

    if "sellersku" in h_lower and "quantity" in h_lower and "sku.skuid" not in h_lower:
        return "Channel_Zalora"

    if "product id" in h_lower and "sku reference no." in h_lower:
        return "Channel_Shopee"

    if "product id" in h_lower and "sku" in h_lower and "sku.skuid" not in h_lower:
        return "Channel_Shopee"

    return None


def detect_file_type_flexible(file_path: str) -> "tuple[str | None, str]":
    """
    Detect file type using filename first, then headers if filename gives no answer.

    Returns (file_type, method):
      file_type : 'Channel_Shopee' | 'Channel_Lazada' | 'Channel_Zalora' |
                  'ordazzle' | 'sap' | None
      method    : 'filename' | 'headers' | 'unknown'
    """
    # ── 1. Try filename ───────────────────────────────────────────────────────
    channel_by_name = detect_channel_from_filename(file_path)
    if channel_by_name:
        return channel_by_name, "filename"

    base_lower = os.path.basename(file_path).lower()
    if base_lower.startswith(("exl", "exp")):
        return "ordazzle", "filename"
    if base_lower.startswith("article"):
        return "sap", "filename"

    # ── 2. Read actual headers from row 1 ────────────────────────────────────
    try:
        ext = os.path.splitext(file_path)[1].lower()
        headers = []
        ...
        if ext in (".xlsx", ".xls"):
            with open(file_path, "rb") as fh:
                content = fh.read()
            headers = scan_xlsx_bytes_headers_generic(content)
            if not headers:
                headers = scan_xlsx_bytes_headers(content)
            ...
            with zipfile.ZipFile(file_path) as zf:
                ...
                if xlsx_names:
                    with zf.open(xlsx_names[0]) as inner:
                        content = inner.read()
                    headers = scan_xlsx_bytes_headers_generic(content)
                    if not headers:
                        headers = scan_xlsx_bytes_headers(content)

        print(f"  [detect_file_type_flexible] {os.path.basename(file_path)!r} "
              f"— row-1 headers: {headers[:5]}")

        if headers:
            ft = detect_channel_from_headers(headers)
            if ft:
                return ft, "headers"

    except Exception as e:
        print(f"  [detect_file_type_flexible] could not read headers from "
              f"{os.path.basename(file_path)!r}: {e}")
