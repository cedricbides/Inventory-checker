...
from ..utils import append_unique_values, is_valid_sku, to_num, merge_row


def _is_simple_sap_format(hdrs_up: set) -> bool:
    """
    Return True when the file looks like the simplified SKU/Stock upload format.

    Simple format has SKU and STOCK columns but no ARTICLE column.
    This check was previously inlined in both scan_sap_columns and read_sap_file.
    """
    return "SKU" in hdrs_up and "STOCK" in hdrs_up and "ARTICLE" not in hdrs_up


def _normalise_simple_headers(raw_hdrs):
    """Return the normalised header list for a simple-format SAP file."""
    return ["Article", "Unrestricted"] + [
    ...
    for path in sap_paths:
        try:
            ...
            for row in ws.iter_rows(values_only=True):
                ...
                if _is_simple_sap_format(hdrs_up):
                    normalised = _normalise_simple_headers(hdrs)
                    append_unique_values(cols, seen, normalised)
    ...
    try:
        ...
        for row in ws.iter_rows(values_only=True):
            if headers is None:
                ...
                if _is_simple_sap_format(hdrs_up):
                    is_simple = True
                    all_headers = _normalise_simple_headers(raw_hdrs)
