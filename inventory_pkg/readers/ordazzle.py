...
from ..utils import get_shared_strings, xml_cell_val, is_valid_sku, merge_row
    ...
    for path in ordazzle_paths:
        try:
            with zipfile.ZipFile(path) as z:
                ...
                for sp in sorted(sheets):
                    with z.open(sp) as ws:
                        root = ET.parse(ws).getroot()
                    ns = {"x": "http://schemas.openxmlformats.org/spreadsheetml/2006/main"}
                    for row in root.findall(".//x:row", ns):
                        cells_list = [xml_cell_val(c, strings, ns)
