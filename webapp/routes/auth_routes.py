...
def _get_ip() -> str:
    """Return the caller's IP, preferring X-Forwarded-For if present."""
...
def _record_fail(ip: str):
    """Record a failed login attempt and lock the IP out if over the limit."""
    record = _fail_tracker[ip]
    now = time.time()
    record["count"] += 1
...
def _clear_fail(ip: str):
    """Clear failed-attempt tracking for an IP after a successful login."""
...
def login_required(f):
    """Decorator: returns 401 if the caller is not logged in."""
    @wraps(f)
...
def admin_required(f):
    """Decorator: returns 401 if not logged in, 403 if not an admin."""
    @wraps(f)
    def decorated(*args, **kwargs):
        if not session.get("logged_in"):
            return jsonify({"error": "Unauthorized", "login_required": True}), 401
...
def api_login():
    ip = _get_ip()
    blocked, secs_remaining = _check_rate_limit(ip)
    if blocked:
        log.warning("Blocked login attempt from %s (%ds remaining)", ip, secs_remaining)
        return jsonify({"error": f"Too many failed attempts. Try again in {secs_remaining} seconds."}), 429
    ...
    if not user_row or user_row["password_hash"] != _hash(password):
        _record_fail(ip)
    ...
    session["permissions"] = user_row["permissions"] or "[]"
    session.permanent = True

    write_audit("login", {"username": username, "role": user_row["role"]})
    log.info("Login: %s (%s)", username, user_row["role"])
...
def api_change_password():
    ...
    if not user_row or user_row["password_hash"] != _hash(current_password):
